    ...
    useEffect(() => {
      ...
      const handleMapClick = (event) => {
        ...
        if (zoom < MIN_QUERY_ZOOM) {
          ...
          const url = "https://livefeeds3.arcgis.com/arcgis/rest/services/GEOGLOWS/GlobalWaterModel_Medium/MapServer/identify";
          if (markerLayer) {
            map.removeLayer(markerLayer);
          }
          const marker = new Feature({type: 'marker', geometry: new Point(coordinate)});
          marker.setStyle(
            new Style({
              image: new Icon({
                src: svgURI,
